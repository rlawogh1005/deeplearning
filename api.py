...
# 위 코드는 모델을 초기화하고 하이퍼파라미터를 로드한다.
# load_ckpt 메서드에서 저장된 모듈의 가중치를 로드한다.
...
# 위 코드는 여러 기능을 가진다.
# 언어 마크, 텍스트 전처리, 오디오 병합, 문장 분할, 음성 합성
...
class ToneColorConverter(OpenVoiceBaseClass):
    ...
    def detect_watermark(self, audio, n_repeat):
        ...
        return message
# 위 코드는 워터마크 모델을 로드하고, add_watermark 메서드를 통해 오디오에 워터마크를 추가한다.
# convert 메서드는 소스 오디오를 로드하고, 음성을 변환하여 결과를 저장한다.
# detect_watermark 메서드는 오디오에서 워터마크를 감지한다.
